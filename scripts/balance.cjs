...
function run(label,delay){const g=E.freshGame(),rows=[];let time=0;for(let biome=0;biome<8;biome++){g.active=biome;const s=g.worlds[biome];let nextAction=0,local=0;for(;local<10800;local++){
 ...
 if(affordable!==undefined&&s.coins>E.price(s,biome)*2+E.upgradePrice(s,affordable))E.buyUpgrade(s,affordable);
 else{const pair=E.pair(s);if(pair)E.merge(s,...pair,biome);else E.add(s,biome,true);}
 }}
 if(s.best>=C.biomes[biome].unlock){E.unlock(g);break;}
 }
