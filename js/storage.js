(() => {
'use strict';
const KEY='amoeba-evolution-v4',BACKUP=KEY+'-backup';
function read(k){try{return localStorage.getItem(k);}catch{return null;}}
function load(){
 let recovered=false;
 for(const key of [KEY,BACKUP,'amoeba-evolution-v3','amoeba-evolution-v2']){
 ...
 legacy.forEach((name,i)=>{try{const old=JSON.parse(read('gameState_'+name));if(!old)return;game.worlds[i]=Evolution.clean({coins:old.coins,creatures:old.amoebas?.map(c=>({level:c.level,x:c.x/1280,y:c.y/720})),discovered:old.discoveredLevels});game.tutorial=true;game.started=true;}catch{}});
